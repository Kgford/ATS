...
class Barcodes(models.Model):
    ...
    def save(self, *args, **kwargs):          # overriding save() 
        ...
        if self.standard =='code39':
            ...
            self.image.save(f'{self.part_number}.png', io.BytesIO(rv.getvalue()), save=False)
            return super().save(*args, **kwargs)
        elif self.standard =='code128':
            COD128 = barcode.get_barcode_class('code128')
            print('File(rv)=',File(rv))
            rv = BytesIO()
            code = COD128(f'{self.part_number}', writer=ImageWriter()).write(rv)
            print('rv=',rv.getvalue())
            print('self.part_number=',self.part_number)
            print('code=',code)
            self.image.save(f'{self.part_number}.png', io.BytesIO(rv.getvalue()), save=False)
            return super().save(*args, **kwargs)
