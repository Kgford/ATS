...
def update_reoccuring_expenses():
    ...
    for desc in expenses:
        ...
        temp_date = str(thisyear) + "-" + str(thismonth)  + "-" + str(day)
        if not Expenses.objects.filter(expense_description=expense_description, sale_date__year=thisyear, sale_date__month=thismonth, sale_date__day=thisday).exists():
