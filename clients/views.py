...
class ClientView(View):
    ...
    def get(self, *args, **kwargs):
        ...
        try:
            ...
            print('error = ',e) 
        return render (self.request,"clients/index.html",{"form": form, "clients":clients, "index_type":"SIGNIN", "UserN":self.request.user, "index_type":"Client", "operator":operator, "active":active})
    ...
    def post(self, request, *args, **kwargs):
        if request.method == 'POST':
            ...
            try: 
                if Clients.objects.filter(name=name).exists():
                    site = Clients.objects.filter(name=name).all()
                    print('site=',site)
                    return redirect('clients:site',site[0].id)
                else:
                    if lat=='':
                        print('lat=',lat)
                        Clients.objects.create(name=name, address=address, city=city, state=state, zip_code=zip_code, phone=phone, email=email, website=website,
        ...
        return render (self.request,"clients/index.html",{"clients":clients, "index_type":"SIGNIN", "UserN":self.request.user, "index_type":"Client" , "operator":operator, "active":active})
...
def site(request,client_id):
    ...
    if request.method == 'GET':
        ...
        try:	
            ...
            print('error = ',e) 
        return render(request,"clients/site.html",{"sites": sites, "site": site, "lat":lat, "lng":lng, "index_type":"Model", "active":active, "operator":operator})
